...
const HighScores = () => {
  ...
  return (
    <div className="relative flex flex-col min-h-screen bg-[#F8FAFC] overflow-hidden font-sans text-[#0F172A]">
      <div className="relative z-10 py-16 px-4 md:px-8 max-w-7xl mx-auto w-full">
        <div className={`text-center mb-20 transition-all duration-1000 transform ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center justify-center p-4 rounded-full bg-yellow-100 mb-6 border border-yellow-200 shadow-sm">
            <FaTrophy className="text-5xl md:text-6xl text-yellow-500" />
          </div>
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter font-display mb-6 text-[#0F172A]">
            Hall of <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-orange-500">Fame</span>
          </h1>
          <p className="text-[#64748B] text-lg md:text-2xl max-w-3xl mx-auto font-medium leading-relaxed">
          ...
          {scoreData.map((category, idx) => (
            ...
            >
              <div className="mb-10 text-center md:text-left">
                ...
                <p className="text-[#64748B] text-lg md:text-xl font-medium">{category.desc}</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
                {category.games.map((game, gIdx) => {
                  ...
                  return (
                    <div 
                      key={gIdx} 
                      className="group relative bg-white rounded-3xl p-6 border border-gray-100 transition-all duration-500 hover:-translate-y-2 shadow-sm hover:shadow-xl flex flex-col h-full"
                    >
                      <div className="flex items-start justify-between mb-8">
                        <div className="flex items-center space-x-4">
                          ...
                          <h3 className="text-xl font-bold font-display text-[#0F172A] leading-tight max-w-[120px]">{game.title}</h3>
                        </div>
                        
                        {hasScore && (
                          <div className="flex flex-col items-end">
                            <span className="text-xs font-bold text-[#64748B] uppercase tracking-wider mb-1">Best Score</span>
                            <div className="flex items-baseline">
                              <span className={`text-3xl font-black ${game.color}`}>
                                {score}
                              </span>
                              {game.unit && <span className="text-[#64748B] ml-1 font-bold text-lg">{game.unit}</span>}
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-auto pt-5 border-t border-gray-100">
                        ...
                        <Link 
                          to={game.path}
                          className="flex items-center space-x-2 px-5 py-2.5 rounded-full bg-[#F8FAFC] hover:bg-blue-50 text-[#2563EB] font-semibold text-sm transition-all border border-gray-200 hover:border-blue-200"
                        >
