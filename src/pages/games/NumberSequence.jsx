  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-[100px])] py-8 bg-[#F8FAFC] px-4 font-sans text-[#0F172A]">
      ...
      <div className="w-full max-w-3xl text-sm mb-6 text-[#64748B] flex items-center space-x-2 mt-12 md:mt-0">
        ...
        <span>Games</span>
        <span>&gt;</span>
        <span className="text-[#0F172A] font-semibold">Number Sequence</span>
      </div>
        ...
        {gameState !== 'menu' && gameState !== 'over' && (
            <div className="flex space-x-2 mb-8 h-4">
                {sequence.map((_, idx) => (
                    <div 
                        key={idx} 
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${idx < userStep ? 'bg-blue-600 shadow-sm scale-125' : 'bg-gray-200'}`}
                    />
                ))}
            </div>
        )}
                    ...
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => {
                        ...
                        return (
                            <button 
                                ...
                                className={`
                                    font-bold text-3xl py-6 rounded-xl border transition-all duration-150
                                    ${isActive 
                                        ? 'bg-blue-600 text-white border-blue-700 shadow-md scale-105' 
                                        : 'bg-gray-50 text-[#0F172A] border-gray-200 hover:bg-gray-100 active:scale-95'}
