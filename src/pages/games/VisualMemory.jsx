  ...
  const renderGrid = () => {
    ...
    for (let i = 0; i < totalTiles; i++) {
        ...
        if (isActuallyPattern || isCorrect || isRevealedPatternOnFail) {
            tileClass += "bg-white border-2 border-blue-500 scale-95 shadow-md z-10";
        } else if (isWrong) {
            tileClass += "bg-red-500 scale-95 shadow-md border-2 border-red-600 z-10 animate-pulse";
        } else {
            tileClass += "bg-blue-500 hover:bg-blue-600 border-b-4 border-blue-700 shadow-sm";
        }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-[100px])] py-8 bg-[#F8FAFC] px-4 font-sans text-[#0F172A]">
      ...
      <div className="w-full max-w-2xl text-sm mb-6 text-[#64748B] flex items-center space-x-2 mt-12 md:mt-0">
        ...
        <span>Games</span>
        <span>&gt;</span>
        <span className="text-[#0F172A] font-semibold">Visual Memory</span>
      </div>
