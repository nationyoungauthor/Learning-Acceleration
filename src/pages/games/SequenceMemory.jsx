...
const SequenceMemory = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-[100px])] py-8 bg-[#F8FAFC] px-4 font-sans text-[#0F172A]">
      {/* Breadcrumbs */}
      <div className="w-full max-w-2xl text-sm mb-6 text-[#64748B] flex items-center space-x-2">
        <Link to="/" className="hover:text-[#2563EB] flex items-center transition-colors"><FaHome className="mr-1"/> Home</Link>
        <span>&gt;</span>
        <span>Games</span>
        <span>&gt;</span>
        <span className="text-[#0F172A] font-semibold">Sequence Memory</span>
      </div>
      ...
      <div className="w-full max-w-xl bg-white rounded-2xl md:p-6 p-4 shadow-md flex flex-col items-center justify-center relative overflow-hidden border border-gray-200 pb-10 min-h-[350px]">
        ...
        <div className="text-center mt-[70px] mb-6 min-h-[70px]">
           <h2 className="text-3xl md:text-4xl font-extrabold tracking-wide font-display mb-2 text-[#0F172A] transition-opacity duration-300">{message}</h2>
           <p className="text-lg md:text-xl text-[#64748B] font-medium">{subMessage}</p>
        </div>
            ...
            {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((index) => (
                <button
                   ...
                   className={`
                      ...
                      ${activeSquare === index ? 'bg-white opacity-90 scale-95 shadow-md border-2 border-blue-500 z-10' : 
                        wrongSquare === index ? 'bg-red-500 scale-95 shadow-md border-2 border-red-600 z-10 animate-pulse' : 
                        'bg-blue-500 hover:bg-blue-600 border-b-4 border-blue-700 shadow-sm'}
